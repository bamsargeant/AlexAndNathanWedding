...
export class MapComponent implements OnInit {
  ...
  constructor(
    ...
    this.apiLoaded = httpClient
      .jsonp(
        'https://maps.googleapis.com/maps/api/js?key=AIzaSyCZxsBsFPOxwrDPUm_m4xPTvKrvJ8bw5eg',
        'callback'
      )
      .pipe(
        map(() => true),
  ...
  ngOnInit(): void {
    setTimeout(() => this.reloadMap(), 50);
  }

  reloadMap() {
    if (!this.map || !this.map.width || !this.width) return;
    this.map.width = this.element.nativeElement.offsetWidth;
    this.map.ngOnInit();
  }
